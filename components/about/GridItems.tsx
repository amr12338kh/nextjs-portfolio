...
const GridItems = ({
  ...
  return (
    <motion.div
      className={cn(
        "row-span-1 h-80 w-full p-4 relative bg-background rounded-2xl overflow-hidden transition-all duration-300 justify-between flex flex-col",
        ...
        "ring-1 ring-muted/40 hover:ring-muted/60 group",
        className,
      )}
    >
      <div>
        ...
        <p className="text-muted-foreground text-sm md:text-xs lg:text-sm max-w-75">
          {des}
        </p>
      </div>
      {children}
